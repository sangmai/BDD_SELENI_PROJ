            ...
            if (size > 0) {
                ...
                if (size == 1) {
                    ...
                    for (int failure = 0; failure < size - 1; failure++) {
                        ...
                        //Sang edit message to Excel report
//                        message.append(Utils.stackTrace(failures.get(failure), false)[0]).append("\n");
                        message.append(failures.get(failure).getMessage()).append("\n");
                    }
                    Throwable last = failures.get(size - 1);
                    message.append("Failure ").append(size).append(" of ").append(size).append(": \n");
                    message.append(last.getMessage());
